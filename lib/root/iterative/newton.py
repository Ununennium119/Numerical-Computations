...
class NewtonRoot:
    """A class used for calculating roots of functions with Newton's method."""

    def __init__(self,
                 initial_guess: float,
                 func: Callable[[float], float],
                 diff: Callable[[float], float],
                 error: float = 10e-7,
                 max_iterations: int = 100):
        self._points: list[float] = [initial_guess]
        self._func: Callable[[float], float] = func
        self._diff: Callable[[float], float] = diff
    ...
    def find_root(self) -> float:
        ...
        for i in range(self._max_iterations):
            ...
            next_point = current_point - self._func(current_point) / self._diff(current_point)
